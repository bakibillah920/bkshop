                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-10 m-auto">
                                <form method="post" action="{{ route('merchant.product.update', $product->id) }}"
                                    ...
                                    <div class="form-group">
                                       <label for="inputStore" class="col-form-label">Store <span
                                           ...
                                           <option value="0" >--Select Store--</option>
                                            ...
                                            <option value="{{ $storeId }}" @if ($product->store_id == $storeId) selected @endif>{{$storename}}</option>
                                            @endforeach
                                        </select>
                                        @error('store_id')
                                    ...
                                    <div class="form-group">
                                       <label for="inputCategory" class="col-form-label">Category <span
                                           ...
                                           <option value="0" >--Select Category--</option>
                                            @foreach ($categoryList as $cateId=>$catname)
                                            <option value="{{ $cateId }}" @if ($product->category_id == $cateId) selected @endif>{{$catname}}</option>
                                            @endforeach
                                        </select>
                                        @error('category_id')
                                            <span class="text-danger">{{ $message }}</span>
